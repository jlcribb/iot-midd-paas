...
`main.py` se conserva como entrypoint `transition/manual-only`.

Ya no es la ruta operativa preferida del sistema. Las rutas canonicas actuales son:

- stack local: `podman compose -f infra/containers/podman-compose.yaml up -d`
