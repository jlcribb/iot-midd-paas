...
def test_handle_event_disabled_emits_only_audit(monkeypatch):
    ...
    monkeypatch.setattr(worker, "publish_event", lambda queue, payload: published.append((queue, payload)))
    monkeypatch.setattr(worker, "_persist_audit_envelope", lambda payload, action: persisted.append((action, payload)))
