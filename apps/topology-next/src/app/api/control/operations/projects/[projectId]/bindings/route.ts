...
import { parseOperationsPage } from "@/lib/http/control-operations-query";
...
export const GET = withRouteErrorHandling(async (request: Request, context: { params: Promise<{ projectId: string }> }) => {
    ...
    return ok(await service.listBindings(actor, projectId, parseOperationsPage(new URL(request.url).searchParams)));
});
