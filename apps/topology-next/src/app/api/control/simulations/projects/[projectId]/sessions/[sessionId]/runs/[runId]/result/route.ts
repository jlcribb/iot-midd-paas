...
export const GET = withRouteErrorHandling(async (request: Request, { params }: RouteParams) => { const actor = await resolveAuthenticatedControlActor(request); const values = await params; return ok(await service.result(actor, uuidSchema.parse(values.projectId), uuidSchema.parse(values.sessionId), uuidSchema.parse(values.runId))); });
